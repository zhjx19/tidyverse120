...
后来，中山大学博士陈熹提供了 R 语言版本。我[^ft1][^ft2]再来个更能体现 R 语言最新技术的 tidyverse 版本。
...
**注：** 此外，`head()`查看前几行，dplyr包还提供了`sample_n()`和`sample_frac()`随机查看n行或某比例的行。
...
**注：** 此外，`tail()`查看后几行，dplyr包还提供了`sample_n()`和`sample_frac()`随机查看n行或某比例的行。
...
**注：** 也可以用sjmisc包中的`rec()`，和SPSS的重新编码一样强大。
...
```{r}
df %>% 
  count(education)   # 或者用table(df$education)
```
...
```{r}
df = readxl::read_xls("51-80数据.xls")
df
```
...
```{r}
df %>% 
  slice(seq(1, n(), by = 20))  # 或者df[seq(1, nrow(df), 20),]
```
...
```{r}
df %>% 
  slice(rev(1:n()))    # 或者 df[rev(1:nrow(df)),]
```
